...
import { CreateTerminalForm, ViewTerminalInformation } from "./modalForm";

export default function Terminals() {
return (
<ChakraProvider>
<Flex className="main-container" justifyContent='space-around' flexWrap='wrap'>
  <Text fontSize='20px' fontWeight='extrabold' textTransform='uppercase' w='100%'>Terminal Dashboard</Text>
  ...
  <Box bg="white" boxShadow='1px 1px 5px gray' w='100%' h='auto' borderRadius='10px' p='1em'>
      <TableContainer >
        <Table size='sm' >
          ...
          <Tbody >
              {terminalList.map(list => (
                <Tr to={list.terminalID} key={list.terminalID} _hover={{ backgroundColor:'gray.100' }}>
                  <Td>{list.terminalID}</Td>
                  <Td>{list.terminalHolder}</Td>
                  <Td>{list.terminalType}</Td>
                  <Td>{list.terminalCardScheme}</Td>
                  <Td>{list.terminalStatus}</Td>
                  <Td>{list.dateCreated}</Td>
                  <Td>{list.dateUpdated}</Td>
                  <Td>{<ViewTerminalInformation />}</Td>
                </Tr>
              ))}
